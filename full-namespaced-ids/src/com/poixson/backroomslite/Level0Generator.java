			...
			for (int x = 0; x < 16; x++) {
				...
				if (isWall) {
					for (int iy = 1; iy < wh; iy++) {
						String wallMaterialString = plugin.getConfig().getString("lobby.walls.material");
						BlockData wallBlockData = Bukkit.createBlockData(wallMaterialString);
						chunk.setBlock(x, this.level_y + iy, z, wallBlockData);
					}
				} else {
					String floorMaterialString = plugin.getConfig().getString("lobby.floor.material");
					BlockData floorBlockData = Bukkit.createBlockData(floorMaterialString);
					chunk.setBlock(x, this.level_y + 1, z, floorBlockData);
					final int modX7 = (xx < 0 ? 1 - xx : xx) % 7;
					final int modZ7 = (zz < 0 ? -zz : zz) % 7;
					if (modZ7 == 0 && modX7 < 2) {
						// ceiling lights
						String lampMaterialString = plugin.getConfig().getString("lobby.lamp.material");
						BlockData lampBlockData = Bukkit.createBlockData(lampMaterialString);
						chunk.setBlock(x, cy, z, lampBlockData);
